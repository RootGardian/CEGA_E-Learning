...
exports.submitExam = exports.getExamData = void 0;
...
const qcmData_1 = require("../utils/qcmData");
...
const sanitizeQuestion = (q) => {
    ...
    if (sanitized.content.options) {
        sanitized.content.options.forEach((opt) => delete opt.isCorrect);
    }
    ...
    if (sanitized.content.items) {
        sanitized.content.items.forEach((item) => delete item.correctOrder);
    }
...
const getExamData = async (req, res) => {
    try {
        ...
        let questions = qcmData_1.qcmBank.filter(q => selectedIds.includes(q.id)).map(sanitizeQuestion);
        // Shuffle questions
...
const submitExam = async (req, res) => {
    try {
        ...
        const selectedIds = evaluation.qcmQuestions || [];
        const questionsToGrade = qcmData_1.qcmBank.filter(q => selectedIds.includes(q.id));
        let totalScore = 0;
        ...
        questionsToGrade.forEach(q => {
            ...
            if (q.type === 'VRAI_FAUX' || q.type === 'QCU') {
                ...
                studentAns.forEach((itemId, index) => {
